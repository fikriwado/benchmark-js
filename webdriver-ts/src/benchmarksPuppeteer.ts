...
export let benchRun = new (class extends CPUBenchmarkPuppeteer {
  ...
  async init(page: Page) { 
    ...
    for (let i = 0; i < config.WARMUP_COUNT; i++) {
      ...
      await checkElementContainsText(page, "pierce/tbody>tr:nth-of-type(1)>td:nth-of-type(1)", (i*volume+1).toFixed());
      await clickElement(page, "pierce/#clear");
      await checkElementNotExists(page, `pierce/tbody>tr:nth-of-type(${volume})>td:nth-of-type(1)`);
    }
  }
  async run(page: Page) {
    await clickElement(page, "pierce/#runlots");
  ...
  async init(page: Page) {
    ...
    await checkElementExists(page, `pierce/tbody>tr:nth-of-type(${volume})>td:nth-of-type(1)`);
      for (let i = 0; i < config.WARMUP_COUNT; i++) {
        ...
        await checkElementContainsText(page, `pierce/tbody>tr:nth-of-type(${volume-9})>td:nth-of-type(2)>a`, ' !!!'.repeat(i + 1));
      }
  }
  async run(page: Page) {
    await clickElement(page, "pierce/#update");
...
export const benchSelect = new (class extends CPUBenchmarkPuppeteer {
  ...
  async init(page: Page) {
    await checkElementExists(page, "pierce/#runlots");
    await clickElement(page, "pierce/#runlots");
    await checkElementContainsText(page, `pierce/tbody>tr:nth-of-type(${volume})>td:nth-of-type(1)`, volume.toString());
    for (let i = 0; i <= config.WARMUP_COUNT; i++) {
...
export const benchSwapRows = new (class extends CPUBenchmarkPuppeteer {
  ...
  async run(page: Page) {
      await clickElement(page, "pierce/#swaprows");
      await checkElementContainsText(page, `pierce/tbody>tr:nth-of-type(${volume-1})>td:nth-of-type(1)`, "2");
      await checkElementContainsText(page, "pierce/tbody>tr:nth-of-type(2)>td:nth-of-type(1)", (volume-1).toString());
  }
})();
...
export const benchAppendToManyRows = new (class extends CPUBenchmarkPuppeteer {
  ...
  async run(page: Page) {
    ...
    await checkElementExists(page, `pierce/tbody>tr:nth-of-type(${volume+1000})>td:nth-of-type(1)`);
  }
})();
...
export const benchCreateClear5Memory = new (class extends MemBenchmarkPuppeteer {
  ...
  async run(page: Page) {
    for (let i = 0; i < 5; i++) {
      await clickElement(page, "pierce/#runlots");
      await checkElementContainsText(page, `pierce/tbody>tr:nth-of-type(${volume})>td:nth-of-type(1)`, (volume * (i + 1)).toFixed());
      await clickElement(page, "pierce/#clear");
      await checkElementNotExists(page, `pierce/tbody>tr:nth-of-type(${volume})>td:nth-of-type(1)`);
    }
  }
